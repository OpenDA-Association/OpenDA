...
/**
 * Created by jsumihar on 2/20/14 by a little bit modifying mverlaan's SimpleLeastSquaresCostFunction:
 *
 * Simple least squares cost function for testing
 *
 * model y=A*p, where dim(y)!= dim(p) a.o. to make it easier to check the orientation of Jacobian matrix.
 * y_obs = Gaussian with mean= [1.0,1.0,1.0] with uncorrelated elements with sigma=[1.0,2.0,3.0]
 * p0 = Gaussian with mean= [0.0,0.0] with uncorrelated elements with sigma=[1.0,2.0]
 * A is equal to Matrix A = new Matrix("[1.0,0.0;0.0,1.0;0.5,0.5]");
 *
 * The solution without constraint is
 * p_opt = [1.0,1.0]
 * Pest = [0.9756,-0.09756;-0.09756,3.609756]
 * cost = 0.0
 * with constraint //TODO fix this
 * p_opt =
 * cost =
 * Pest =
 */
public class SimpleLeastSquaresCostFunctionWithState extends SimulationKwadraticCostFunction {
	//fields for this class
	private IStochVector obs = new StochVector("[1.0,1.0,1.0]","[1.0,2.0,3.0]");
	private IStochVector par = new StochVector("[0.5,0.5]","[1.0,2.0]");
	...
	public SimpleLeastSquaresCostFunctionWithState(IStochModelFactory modFac, IStochObserver obs) {
		super(modFac, obs);
	}

	public SimpleLeastSquaresCostFunctionWithState(){
		numberEvaluations = 0;
	}
	...
	public double evaluate(IVector p, String context) { // Cost = Sum_i ((p(i)-means(i)/width(i))^2
		...
		//map
		// y=A*p , i.e. very simple model!
		IVector state = new Vector(3);
		Matrix A = new Matrix("[1.0,0.0;0.0,1.0;0.5,0.5]");
		A.rightMultiply(1.0,p,0.0,state);
		this.pred = state.clone();
	...
	public IVector getLastPredictions(){
		IVector result = this.pred.clone();
		return result;
	}
	...
	public LeastSquaresCostFunction clone(){
		SimpleLeastSquaresCostFunctionWithState result = new SimpleLeastSquaresCostFunctionWithState();
		return result;
