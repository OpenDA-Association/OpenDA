      ...
      ELSE  
        ...
        ELSE  
          DO K=1,KC  
            DO L=2,LA  
              WB=0.25*DXYP(L)*(W2(L,K-1)+W2(L,K))  
              FWQQ(L,K)=MAX(WB,0.)*TVAR2C(L,K-1)  
     &            +MIN(WB,0.)*TVAR2C(L,K)  
        ...
        ELSE  
          DO K=1,KS  
            DO L=2,LA  
              ...
              UHUW=0.25*(UHDY2(L,K)+UHDY2(L,K+1))  
              VHVW=0.25*(VHDX2(L,K)+VHDX2(L,K+1))  
              FUHU(L,K)=MAX(UHUW,0.)*TVAR2C(L-1,K)  
     &            +MIN(UHUW,0.)*TVAR2C(L,K)  
              FVHU(L,K)=MAX(VHVW,0.)*TVAR2C(LS,K)  
     &            +MIN(VHVW,0.)*TVAR2C(L,K)  
...
C  
     ...
     &            +AV(L,K)*DZIGSD4(K)*(U(L+1,K+1)-U(L+1,K)+U(L,K+1)-
     &      U(L,K))**2+AV(L,K)*DZIGSD4(K)*(V(LN,K+1)-V(LN,K)+V(L,K+1)
