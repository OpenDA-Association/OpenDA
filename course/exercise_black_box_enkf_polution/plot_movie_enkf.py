...
n_noise_enkf = len(enkf.x_a[0])-2*ngrid

print("Number of noise parameters in EnKF model run"+str(n_noise_enkf))
print("no_sources="+str(no_sources))
...
for i in range(len(enkf.analysis_time)):
   ...
   ax[0].plot(c1_sim,'b')
   c1_enkf=enkf.x_a[i,(n_noise_enkf):(no_sources+ngrid)]
   ax[0].plot(c1_enkf,'g')
   ...
   ax[1].plot(c2_sim,'b')
   c2_enkf=enkf.x_a[i,(n_noise_enkf+ngrid):(n_noise_enkf+2*ngrid)]
   ax[1].plot(c2_enkf,'g')
