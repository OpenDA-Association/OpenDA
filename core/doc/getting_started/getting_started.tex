...
For the examples \verb|model_dflowfm_blackbox| and \verb|model_delft3d| in this folder, it is necessary to connect to existing models. 
There is a readme available which explains some extra steps which might be needed, like installing the D-Flow FM software.
...
The \oda source code can be cloned from the \oda \href{https://github.com/OpenDA-Association/OpenDA.git}{GitHub page}.

\section{Development with IntelliJ IDEA}
The easiest way to develop on the \oda Java source code is using the IntelliJ IDEA.
The free community version can be downloaded from: \href{https://www.jetbrains.com/idea/download/}{https://www.jetbrains.com/idea/download/}.
By opening the \verb|openda.ipr| with IntelliJ most of the sources will already be setup correctly.

Make sure there is at least a 64-bit jdk 1.8 as a project SDK.
To build the source code into usable binaries, Ant is used which is already present in recent versions of IntelliJ.
By opening the \verb|build.xml| in the Ant plugin, a list of build targets can be run.
