 ...
 */
public class NetcdfResultWriterNative extends CtaObject implements IResultWriter {
    ...
    public NetcdfResultWriterNative(File workingDir, String configString) {
        if (configString.startsWith("<xml")) {  // TODO: right prefix
            // TODO: read from config file
        	throw new RuntimeException("NetcdfResultWriterNative: expecting a filename, but found xml-code");
        } else {
            if (configString.toLowerCase().endsWith(".nc")) {
            	...
            	throw new RuntimeException("NetcdfResultWriterNative: expecting a filename ending on .nc,"
	...
	private static void writeObjectToNetCdfFile(int aCtaFileHandle, Object obj) {
		if (obj instanceof CtaTreeVector) {
			...
			if (retval != 0) {
				throw new RuntimeException("NetcdfResultWriterNative: putValue for CtaTreeVector not succeeded ");
			}
		}
			...
			if (retval != 0) {
				throw new RuntimeException("NetcdfResultWriterNative: putValue for Matrix not succeeded ");
			}

		} else if (obj instanceof CtaVector) {
			int retval = ((CtaVector) obj).export(aCtaFileHandle);
			if (retval != 0) {
				throw new RuntimeException("NetcdfResultWriterNative: putValue for CtaVector not succeeded ");
			}
			...
			int retval = ctaTreeVector.export(aCtaFileHandle);
			if (retval != 0) {
				throw new RuntimeException("NetcdfResultWriterNative: putValue for TreeVector not succeeded ");
			}
		} else if (obj instanceof IVector) {
			CtaVector ctaVector = new CtaVector( (IVector) obj);
			int retval = ctaVector.export(aCtaFileHandle);
			if (retval != 0) {
				throw new RuntimeException("NetcdfResultWriterNative: putValue for Vector not succeeded ");
			}
			...
			throw new RuntimeException("NetcdfResultWriterNative: putValue for this object not implemented: " +
    ...
    public void free() {
        ...
        for (String key2 : idlist.keySet()) {
            ...
            if (retval != 0) {
                throw new RuntimeException("NetcdfResultWriterNative: closing the netcdf file has not succeeded ");
	...
	public static int createNetCdfHandle(String netcdfname, String action) {
		NetcdfResultWriterNative writer = new NetcdfResultWriterNative();
	...
	public NetcdfResultWriterNative() {
