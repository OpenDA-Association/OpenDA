...
/**
 * Class implementing a correlation model with a pentaspherical structure
 * for autocorrelation.
 *
 * This class is derived from DUE version 3.0 by brown@science.uva.nl
 */
public class PentasphericalCorrelation extends AutoCorrelationFunction {

	//distribution name.
	private static final String DISTRIBUTION_NAME = "Pentaspherical";


	/**
	 * Returns a new instance of this correlation object for cloning purposes.
	 *
	 * @return new instance of PentasphericalCorrelation.
	 */
	public PentasphericalCorrelation newInstance() {
        return new PentasphericalCorrelation();
    }

	/**
	 * Construct a pentaspherical correlation model with default values for the parameters.
	 */
	public PentasphericalCorrelation() {
		super();
	...
	/**
	 * Construct an isotropic pentaspherical correlation model with a sill and range as input parameters
	 *
	 * @param sill the sill parameter
	 * @param range the range parameter
	 * @throws IllegalArgumentException if range <= 0
	 */
	public PentasphericalCorrelation(double sill, double range) throws IllegalArgumentException {
		super();
	...
	public double evaluateFunction(double lagDistance) throws ArithmeticException, IllegalArgumentException {
        ...
        if (orderOfAnisotropy == ISOTROPIC) {
            if (lagDistance < range) {
            	return sill * ( ( (15.0 * lagDistance) / (8.0 * range) ) - ( (5.0/4.0) * Math.pow(lagDistance/range, 3 ) ) + ( (3.0/8.0) * Math.pow(lagDistance/range, 5) ) );
            }
            else {
            	return sill * 1.0;
            }
