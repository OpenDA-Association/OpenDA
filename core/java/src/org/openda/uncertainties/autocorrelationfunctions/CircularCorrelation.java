...
/**
 * Class implementing a correlation model with a circular structure
 * for autocorrelation.
 *
 * This class is derived from DUE version 3.0 by brown@science.uva.nl
 */
public class CircularCorrelation extends AutoCorrelationFunction {

	//distribution name.
	private static final String DISTRIBUTION_NAME = "Circular";


	/**
	 * Returns a new instance of this correlation object for cloning purposes.
	 *
	 * @return new instance of CircularCorrelation.
	 */
	public CircularCorrelation newInstance() {
        return new CircularCorrelation() ;
    }

	/**
	 * Construct a circular correlation model with default values for the parameters.
	 */
	public CircularCorrelation() {
		super();
	...
	/**
	 * Construct an isotropic circular correlation model with a sill and range as input parameters
	 *
	 * @param sill the sill parameter
	 * @param range the range parameter
	 * @throws IllegalArgumentException if range <= 0
	 */
	public CircularCorrelation(double sill, double range) throws IllegalArgumentException {
		super();
	...
	public double evaluateFunction(double lagDistance) throws ArithmeticException, IllegalArgumentException {
        ...
        if (orderOfAnisotropy == ISOTROPIC) {
            if (lagDistance < range) {
            	return sill * ( ( (2.0 * lagDistance) / (Math.PI * range) ) * Math.sqrt( 1.0-( Math.pow(lagDistance/range, 2) ) ) + ( (2.0 / Math.PI) * Math.asin(lagDistance/range) ) );
            }
            else {
            	return sill * 1.0;
            }
