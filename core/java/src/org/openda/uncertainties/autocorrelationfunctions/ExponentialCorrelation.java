...
/**
 * Class implementing a correlation model with an exponential structure
 * for autocorrelation.
 *
 * This class is derived from DUE version 3.0 by brown@science.uva.nl
 */
public class ExponentialCorrelation extends AutoCorrelationFunction {
	 ...
	 *
	 * @return new instance of ExponentialCorrelation.
	 */
	public ExponentialCorrelation newInstance() {
        return new ExponentialCorrelation() ;
    }

	/**
	 * Construct an exponential correlation model with default values for the parameters.
	 */
	public ExponentialCorrelation() {
		super();
	...
	/**
	 * Construct an isotropic exponential correlation model with a sill and range as input parameters
	 *
	 * @param sill the sill parameter
	 * @param range the range parameter
	 * @throws IllegalArgumentException if range <= 0
	 */
	public ExponentialCorrelation(double sill, double range) throws IllegalArgumentException {
		super();
	...
	public double evaluateFunction(double lagDistance) throws ArithmeticException, IllegalArgumentException {
        ...
        if (orderOfAnisotropy == ISOTROPIC) {
            return sill * (1.0 - Math.exp(-(lagDistance/range)));
        }
        else {
