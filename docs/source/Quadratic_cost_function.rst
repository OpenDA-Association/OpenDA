...
.. math::
   L(x) = \sum_{i=1}^I \sum_{t=1}^T \frac{(y_{t,i} - f_{t,i}(x))^2}{\sigma_i^2},

where :math:`y` is the set of observations, and :math:`f(x)` the corresponding
set of model results, and :math:`\sigma_i` the user-defined standard deviation
of the observations at location :math:`i`.
...
#. ``stdRemoval="true"``
   ...
   observations is removed from the calculation.  This is not the same as the
   standard deviation of the observations :math:`\sigma_i`.

   For each station, the bias is computed as the average difference between the
   model results and observations over all times: 

   .. math::
      \mathrm{bias}_i =  \frac{\sum_{t=1}^T (y_{t,i} - f_{t,i}(x))}{T}.

   In the cost function, the model-observation difference is replaced by this
   bias:

   .. math::
     L_{\mathrm{bias}}(x) = \sum_{i=1}^I \sum_{t=1}^T
     \frac{\mathrm{bias}_i^2}{\sigma_i^2} = \sum_{i=1}^I T
   ...
   columns have the following meaning: 
     ...
     - *Num of data*: the number of data points at the observation location;
       that are included in the cost calculation (:math:`T`);
   ...
   been used.  It needs 9 iteration to arrive at a minimum cost, generating the
   following output::
...
#. ``biasRemoval="true"``
   ...
   .. math::
      L_{\mathrm{std}}(x) = \sum_{i=1}^I \sum_{t=1}^T \frac{(y_{t,i} -
...
#. ``weakParameterConstraint="true"``
   ...
   .. math::
      L_{\mathrm{weakParam}}(x) = \sum_{i=1}^I \sum_{t=1}^T \frac{( y_{t,i} -
      f_{t,i}(x))^2}{\sigma_i^2} + \sum_{x=1}^X \frac{(\mathrm{ln}(x) -
      \mathrm{ln}(x_0))^2}{\sigma_x^2},

   and this expression is also multiplied by the chosen factor.
