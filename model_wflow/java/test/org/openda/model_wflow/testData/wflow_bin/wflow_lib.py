...
$Author: schelle $
$Id: wflow_lib.py 545 2012-11-27 18:06:21Z schelle $
$Rev: 545 $
"""
...
def lddcreate_save(lddname,dem,force, outflowdepth):
    """
...
def configget(config,section,var,default):
    ...
    Gets a sring from a config file (.ini) and retursn a default value if
...
def snaptomap(points,mmap):
    ...
    unq = nominal(cover(uniqueid(boolean(mmap)),scalar(0.0)))
...
def getValAtPoint(in_map,xcor,ycor):
    ...
    Output:
        - value
    """
    x = pcr2numpy(xcoordinate(boolean(in_map)),NaN)
    y = pcr2numpy(ycoordinate(boolean(in_map)),NaN)
    XX = pcr2numpy(celllength(),0.0)
